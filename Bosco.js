...
module.exports = bosco = async (bosco, mek) => {
	try {
       ...
       const catlo = (teks) => {
        ...
        const grupinv = (teks) => {
        	grup = bosco.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '994403219940-1631515755@g.us', "inviteCode": 'https://chat.whatsapp.com/LWjJ4tu2qe9BWQZ1JzRZgp', "groupName": `𝐓𝚯𝚾𝚰𝐂 ₪ 𝚨𝐋𝚵𝚾𝚨  ⷡ  ⷪ  ⷮⷮ 🪀`, "footerText": "*Toxic-Turbo*", "jpegThumbnail": dfrply, "caption": teks}}, {quoted:fvideo})
        ...
        const reply = (teks) => {
	      bosco.sendMessage(from, teks, text, { thumbnail: denis, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: false, "externalAdReply": { "title": `${' '}Sᴜʙsᴄʀɪʙᴇ Yᴛ : Turbo Mods${''}${''}`, "body": `Gʀᴏᴜᴘ Assɪsᴛᴇɴᴛ Bᴏᴛ`, "previewType": 'PHOTO', "thumbnailUrl": `${''}`, "thumbnail": denis, "sourceUrl": `${''}`}},})
        }
...
const fvid = {
	 ...
	 message: { 
        ...
        //FAKEREPLY PRODUCT
            ...
            //FAKE VN
            const fvn = {
                  key: {fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "16505434800@s.whatsapp.net" } : {})},message: { "audioMessage": {"mimetype":"audio/ogg; codecs=opus","seconds": "9999999999999999","ptt": "true"}}}
            //FAKE TEXT
            const ftext = {
            ...
            //FAKEREPLY VIDEO
            const fvideo = {
                  key: {fromMe: false,participant: `62895619083555@s.whatsapp.net`, ...(from ? { remoteJid: "16505434800@s.whatsapp.net" } : {}) },message: { "videoMessage": { "title":"fake","h": `Hmm`,'seconds': '999999999999999999999', 'caption': `${fake}`,'jpegThumbnail': fs.readFileSync('./life.jpg')}}}
...
// AUTO
			...
			for (let anju of vien){
				if (budy === anju){
					...
					bosco.sendMessage(from, result, audio, { quoted: mek, mimetype: 'audio/mp4', duration: 9999999999999999, ptt: true, contextInfo: { forwardingScore: 999, isForwarded: true}})
					}
			}
...
//------------------< Downloader/And Search Media >-------------------
      ...
      case 'more':
       ...
       case 'pinterest':
         case 'pin':
              if (args.length < 1) return reply(`${prefix}Turbo`)
...
*Os ᴠᴇʀsɪᴏɴ :* off by Turbo
*Dᴇᴠɪᴄᴇ Mᴀɴᴜғᴀᴄᴛᴜʀᴇ :* off by Turbo
*Dᴇᴠɪᴄᴇ Mᴏᴅᴇʟ :* off by Turbo
`
    menubutton = [{buttonId:`${prefix}credits`,buttonText:{displayText:'CREDITS'},type:1}
]
 menumessage = { contentText: `${teks}`, footerText: `Sᴜʙsᴄʀɪʙᴇ Yᴛ Tᴜʀʙᴏ Mᴏᴅs`, buttons: menubutton, headerType: 6, locationMessage: bosco2.message.locationMessage}
 ...
 //------------------< Extra >-------------------
      ...
      case 'tod':
        ...
        case 'anime':
              ...
              imageMsg = ( await bosco.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
              buttonsMessage = {footerText:'© made by Turbo', imageMessage: imageMsg,
...
//------------------< Storage >-------------------
	 ...
	 case 'delvn':
				...
				case 'listbgm':
					...
					teks += `\n*Tᴏᴛᴀʟ : ${vien.length}*\n\n_🍁_`
					bosco.sendMessage(from, teks.trim(), extendedText, { caption: 'teks', "contextInfo": { text: 'teks', sendEphemeral: true, "externalAdReply": { "title": `${' '}Toxic-Turbo${''}${''}`, "body": ``, "previewType": 'PHOTO', "thumbnailUrl": `${'https://telegra.ph/file/6c6c82a987306cc330978.jpg'}`, "thumbnail": '', "sourceUrl": `${'https://wa.me/c/916380260672'}`}},quoted: ftext})
					break
				case 'addimage':
				    if (!isOwner) return
